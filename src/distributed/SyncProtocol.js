...
class SyncProtocol {
  constructor(options) {
    ...
    if (this.persistence?.autoLoad) {
      void this.loadFromPersistence().catch((error) => {
        logger_1.logger.error('[SyncProtocol] Failed to load persistence', {
  ...
  configureCrypto(provider, config) {
    ...
    logger_1.logger.debug('[SyncProtocol] Crypto configured', {
  ...
  async createAuthenticatedHandshake(capabilities, targetDID) {
    ...
    logger_1.logger.debug('[SyncProtocol] Authenticated handshake created', {
  ...
  async verifyAuthenticatedHandshake(message) {
    ...
    if (this.cryptoConfig.requireSignatures && message.auth?.signature) {
      ...
      if (!isValid) {
        logger_1.logger.warn(
          '[SyncProtocol] Handshake signature verification failed',
          {
            messageId: message.messageId,
    ...
    if (this.cryptoConfig.requireCapabilities && handshake.ucan) {
      ...
      if (!result.authorized) {
        logger_1.logger.warn(
          '[SyncProtocol] Handshake UCAN verification failed',
          {
            messageId: message.messageId,
            error: result.error,
          }
    ...
    this.handshakes.set(message.sender, handshake);
    this.schedulePersist();
    logger_1.logger.debug('[SyncProtocol] Authenticated handshake verified', {
      messageId: message.messageId,
      did: handshake.did,
    });
    return { valid: true, handshake };
  ...
  async signMessage(message, payload, encrypt = false) {
    ...
    if (
      ...
      logger_1.logger.debug('[SyncProtocol] Message encrypted', {
        messageId: message.messageId,
  ...
  async verifyMessage(message) {
    ...
    if (message.auth.encrypted && message.payload) {
      try {
        ...
        logger_1.logger.debug('[SyncProtocol] Message decrypted', {
          messageId: message.messageId,
        });
      } catch (error) {
        return {
  ...
  createHandshakeMessage(nodeId, capabilities) {
    ...
    this.messageQueue.push(message);
    this.schedulePersist();
    logger_1.logger.debug('[SyncProtocol] Handshake message created', {
      messageId: message.messageId,
      nodeId,
      capabilities: capabilities.length,
    });
  ...
  createSyncRequestMessage(
    ...
    this.messageQueue.push(message);
    this.schedulePersist();
    logger_1.logger.debug('[SyncProtocol] Sync request created', {
      messageId: message.messageId,
      sessionId,
      fromVersion,
      toVersion,
  ...
  createSyncResponseMessage(
    ...
    this.messageQueue.push(message);
    this.schedulePersist();
    logger_1.logger.debug('[SyncProtocol] Sync response created', {
      messageId: message.messageId,
      sessionId,
  ...
  createErrorMessage(sender, receiver, error, relatedMessageId) {
    ...
    this.schedulePersist();
    logger_1.logger.error('[SyncProtocol] Error message created', {
      messageId: message.messageId,
  ...
  serializeMessage(message) {
    try {
      ...
      logger_1.logger.error('[SyncProtocol] Message serialization failed', {
        messageId: message.messageId,
        error: error instanceof Error ? error.message : String(error),
      });
      throw new Error(
  ...
  deserializeMessage(data) {
    try {
      ...
      logger_1.logger.error('[SyncProtocol] Message deserialization failed', {
        error: error instanceof Error ? error.message : String(error),
      });
      throw new Error(
  ...
  processHandshake(message) {
    ...
    this.schedulePersist();
    logger_1.logger.debug('[SyncProtocol] Handshake processed', {
      nodeId,
  ...
  async loadFromPersistence() {
    ...
    logger_1.logger.debug('[SyncProtocol] Loaded from persistence', {
      key: this.persistence.key,
  ...
  async persistSafely() {
    ...
    try {
      ...
      logger_1.logger.error('[SyncProtocol] Persistence write failed', {
        key: this.persistence.key,
        error: error instanceof Error ? error.message : String(error),
      });
    } finally {
...
SyncProtocol.DEFAULT_PERSIST_KEY = 'aeon:sync-protocol:v1';
