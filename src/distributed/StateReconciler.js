...
exports.StateReconciler = void 0;
...
class StateReconciler {
  ...
  configureCrypto(provider, requireSigned = false) {
    ...
    logger_1.logger.debug('[StateReconciler] Crypto configured', {
  ...
  async recordSignedStateVersion(key, version, data) {
    ...
    logger_1.logger.debug('[StateReconciler] Signed state version recorded', {
      key,
      version,
      signerDID: localDID,
  ...
  async reconcileWithVerification(key, strategy = 'last-write-wins') {
    ...
    for (const version of versions) {
      ...
      if (result.valid) {
        ...
        logger_1.logger.warn('[StateReconciler] Version verification failed', {
          version: version.version,
  ...
  recordStateVersion(key, version, timestamp, nodeId, hash, data) {
    ...
    logger_1.logger.debug('[StateReconciler] State version recorded', {
      key,
      version,
      nodeId,
      hash,
  ...
  reconcileLastWriteWins(versions) {
    ...
    logger_1.logger.debug(
      '[StateReconciler] State reconciled (last-write-wins)',
      {
  ...
  reconcileVectorClock(versions) {
    ...
    this.reconciliationHistory.push(result);
    logger_1.logger.debug('[StateReconciler] State reconciled (vector-clock)', {
  ...
  reconcileMajorityVote(versions) {
    ...
    this.reconciliationHistory.push(result);
    logger_1.logger.debug(
      '[StateReconciler] State reconciled (majority-vote)',
      {
