...
- `lean/Lean/ForkRaceFoldTheorems/Axioms.lean`: explicit-assumption theorem schemas for global claims, including convergence in the modeled finite class, stronger queue-limit shells, a state-dependent open-network stationary/terminal balance schema whose positive-recurrence and stationary-law layer is derived from explicit kernel bridge, drift, and finite-small-set witnesses, and an adaptive supremum-kernel shell that now derives the expected-Lyapunov ceiling from raw ceiling domination data and supports five drift routes: an automatically synthesized minimum-slack bottleneck selector, a raw-score normalization route, a positive-part normalization route for arbitrary real scores, an explicit selector-based one-hot decomposition, or a normalized nonnegative weighted decomposition.
...
- The generic adaptive lift still remains honest about its boundary: constructive domination by the supremum or another dominating kernel is proved in `JacksonQueueing.lean`, `Axioms.AdaptiveCeilingDriftSynthesis` now supports a built-in minimum-slack selector route, a raw-score normalization route, a positive-part normalization route for arbitrary real scores, and the earlier explicit selector-based and normalized weighted routes, and the remaining open part is automatic discovery of richer chosen-Lyapunov decompositions for arbitrary adaptive Lyapunov choices beyond those supported forms.
- The Jackson-network gap is narrower now: `JacksonQueueing.lean` extracts `maxExternalArrival` and `maxIncomingRoutingMass` from raw data, organizes the current witness family as a finite-step envelope ladder `throughputEnvelopeApprox n`, with `n = 0` the global envelope, `n = 1` the local nodewise envelope, and `n = 2` the second-order envelope, supports direct nodewise service bounds on top of that ladder, and also packages the `minServiceRate` corollary as a one-line automatic witness constructor. The remaining open part is deriving still-sharper arrival/routing envelopes or other local certificates beyond that current finite-step ladder.
- On the compiler side, the next open boundary is no longer finite/countable certified-kernel stability; it is the measurable Harris recurrence / geometric-ergodicity bridge for Betti-emitted symbolic queue families.
